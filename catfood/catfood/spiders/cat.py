...
class CatSpider(scrapy.Spider):
    ...
    def printwhatever(self, response):
        ...
        quality_ingredients = response.xpath("//div[contains(@class, 'panel-success')]/div[contains(@class, 'panel-body')]/ul/li/text()").extract()
        l.add_value('quality_ingredients', quality_ingredients)
        questionable_ingredients = response.xpath("//div[contains(@class, 'panel-danger')]/div[contains(@class, 'panel-body')]/ul/li/text()").extract()
        l.add_value('questionable_ingredients', questionable_ingredients)
        potential_allergens = response.xpath("//div[contains(@class, 'panel-warning')]/div[contains(@class, 'panel-body')]/ul/li/text()").extract()
